...
# loop for different pileup
 for i in $(seq 1 16); 
	...
	# ./build/bin/ActsExamplePythia8  --events=500 --rnd-seed=$seed  --output-dir=build/data/gen/ttbar_mu$pu --output-csv --output-root --gen-cms-energy-gev=14000 --gen-hard-process=Top:qqbar2ttbar=on --gen-npileup=$pu
	...
	# rm build/data/reco_generic/ttbar_mu$pu/trackstates_fitter.root

	# rm -rf build/data/sim_generic/ttbar_mu$pu
...
hadd merged.root ttbar_mu*/*.root
mv *.root ..
