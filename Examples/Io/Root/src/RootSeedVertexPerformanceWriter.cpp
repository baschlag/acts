...
ActsExamples::RootSeedVertexPerformanceWriter::RootSeedVertexPerformanceWriter(
    const ActsExamples::RootSeedVertexPerformanceWriter::Config& cfg,
    Acts::Logging::Level lvl)
    : WriterT(cfg.inputVertices, "RootSeedVertexPerformanceWriter", lvl),
      m_cfg(cfg),
...
ActsExamples::ProcessCode ActsExamples::RootSeedVertexPerformanceWriter::endRun() {
  if (m_outputFile) {
    m_outputFile->cd();
    m_outputTree->Write();
  }
...
int ActsExamples::RootSeedVertexPerformanceWriter::
...
ActsExamples::ProcessCode ActsExamples::RootSeedVertexPerformanceWriter::writeT(
      ...
      ctx.eventStore.get<std::vector<Acts::BoundTrackParameters>>(
        ...
        for (const auto& particle : associatedTruthParticles) {
          ...
          if (secVtxId != 0) {
            ...
            std::cout << "pos reco/true: " << vertices[0].position()[2] << ", " << truePos[2] << std::endl;
            m_diffz.push_back(vertices[0].position()[2] - truePos[2]);
            // Next vertex now
            break;
          }
  ...
  m_nTracks = seedTimeMS; // nasty hack: use seedtime variable to store N input tracks to finder
