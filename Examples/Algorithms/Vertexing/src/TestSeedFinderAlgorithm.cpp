...
#include "ActsExamples/Vertexing/TestSeedFinderAlgorithm.hpp"
...
ActsExamples::TestSeedFinderAlgorithm::
    TestSeedFinderAlgorithm(const Config& cfg,
...
ActsExamples::TestSeedFinderAlgorithm::execute(
  ...
  using SeedFinder = Acts::GridDensityVertexFinder<5000, 35>;
  SeedFinder::Config seedFinderCfg(250);
  SeedFinder seedFinder(seedFinderCfg);

  SeedFinder::State state;
  ...
  auto result = seedFinder.find(inputTrackPointers, finderOpts, state);
  auto t2 = std::chrono::high_resolution_clock::now();

  int seedTime = inputTrackPointers.size();//state.totalSeedingTime; // mirco seconds
